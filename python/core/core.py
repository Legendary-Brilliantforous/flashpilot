...
def _wait_for_adb(ctx, log, timeout=60, key=None):
    if key is None:
        ...
        key = _dev.current_key() or ctx.get("device_key")
    want_serial = key[4:] if isinstance(key, str) and key.startswith("adb:") else None
...
def _download_mode_device(key=None):
    ...
    interface must be absent. Accepts all known Odin PIDs (685d/6601/68c3/
    68ef/4eee/4eef) so A02s/SDM450 variants are not missed. ``key`` selects
    one of several connected phones (ambient thread scope by default)."""
    if key is None:
        from . import devices as _dev

        key = _dev.current_key()
    d = mtp.find_samsung(key=key)
    if not d or d.get("pid") not in _ODIN_PIDS:
        return None
...
def _prepare_download_session(d, log, timeout=15):
    """Stabilize an Odin download-mode session before any open/flash.

    Does, in order: kernel-driver detach (cdc_acm steals bulk endpoints and
    causes 'bulk read timed out' / 'usb device Fail'), one live odin_connect
    probe with retry across USB re-enumeration (bus:addr can change), and
    returns a fresh target string. Raises RuntimeError if no session.
    """
    import time as _t
    from . import devices as _dev
    ...
    if isinstance(d, dict):
        target = f"04e8:{d.get('pid', 0x685d):04x}@{d.get('bus')}:{d.get('address')}"
    elif isinstance(d, str):
        target = d
    else:
        nd = _download_mode_device(key=_dev.current_key())
        if not nd:
            raise RuntimeError("phone not in download mode (no 04e8 Odin PID without ADB)")
        target = f"04e8:{nd['pid']:04x}@{nd['bus']}:{nd['address']}"
    # 1. Detach kernel drivers so libusb can claim bulk endpoints.
    try:
        res = bridge.usb_detach_kernel(target, timeout=timeout)
        if log:
            try:
        ...
        if log:
            log(f"  (kernel detach skipped: {e})")
    # 2. Probe handshake with retry; re-resolve target on re-enumeration.
    last_err = None
    for attempt in range(3):
        try:
            bridge.odin_connect(target, timeout=timeout)
            return target
        except bridge.BridgeError as e:
            last_err = e
            if log:
                log(f"  Odin handshake attempt {attempt + 1}/3 failed: {e}")
        except Exception as e:
            last_err = e
            if log:
                log(f"  Odin handshake attempt {attempt + 1}/3 failed: {e}")
        ...
        if nd:
            target = f"04e8:{nd['pid']:04x}@{nd['bus']}:{nd['address']}"
            if log:
                log(f"  Re-resolved download target: {target}")
            try:
                res = bridge.usb_detach_kernel(target, timeout=timeout)
            except Exception:
                pass
    raise RuntimeError(
        f"no Odin session: handshake failed 3x ({last_err}). "
        "Try `sudo rmmod cdc_acm`, replug, re-enter Download Mode (Vol Down + Power, Vol Up)."
    )


def wait_clean_download_mode(log_fn=None, timeout=3600, tag="smart", key=None):
    ...
    ``key`` pins the wait to one phone (ambient thread scope by default).
    """
    if key is None:
        from . import devices as _dev
    ...
    prompted = False
    while time.time() < deadline:
        nd = _download_mode_device(key=key)
...
def pit_contract(target=None, log=None, use_cache_on_failure=True, key=None):
    ...
    ``key`` pins the device lookup to one phone (ambient scope by default).
    """
    if key is None:
        from . import devices as _dev
    ...
    d = _download_mode_device(key=key)
    raw = None
    from_cache = False
    path = None
    ...
    if target:
        # Detach kernel drivers first so the bulk endpoints can be claimed
        # (fixes 'bulk read timed out' / 'device not found' on cdc_acm hosts).
        try:
            bridge.usb_detach_kernel(target, timeout=15)
        except Exception:
            pass
        try:
            ...
            # One retry after re-resolving (bus:addr shifts on re-enumeration).
            try:
                import time as _t
                _t.sleep(2.0)
                nd = _download_mode_device()
                if nd:
                    ...
                    resp = json.loads(bridge.odin_pit(target))
                    raw = bytes.fromhex(resp.get("hex", ""))
                    _log(f"  [pit-contract] retry fetch OK on {target}")
            except Exception as e2:
                _log(f"  [pit-contract] retry fetch failed: {e2}")
    ...
    try:
        ...
        while True:
            ...
            if "done" in resp:
                ...
                elapsed = max(0.1, time.time() - bulk_start)
                rate = wrote / elapsed / 1024 / 1024
                eta = int((total_bytes - wrote) / max(1, wrote) * elapsed) if wrote else 0
                _log(f"[smart]   [{pct:3d}%] {pname} done "
                     f"({resp.get('progress','')}) {rate:.1f} MB/s ETA {eta // 60}m{eta % 60:02d}s")
...
def _wait_download_mode(log, timeout=30, key=None):
    ...
    instead of being in download mode. ``key`` pins the wait to one phone."""
    if key is None:
        from . import devices as _dev
...
def _try_enable_adb(ctx, log):
    ...
    if d is not None and d.get("configs", 1) >= mtp.DIAG_CONFIG:
        log("  [Vector 1] Trying MTP/AT diag-switch to enable ADB ...")
        try:
        ...
        if t is not None and mtp.ping(t, attempts=6):
            ...
            return True
        log("    diag port did not answer - trying strong vectors.")

    # ------------------------------------------------------------------
    # Strong vectors for single-config MTP (like A02s SM-A025 04e8:6860)
    # where the classic DIAG_CONFIG switch is unavailable / patched.
    # ------------------------------------------------------------------
    if d is not None:
        pid = d.get("pid", 0)
        log(f"  [Vector 2] Single-config MTP (pid 0x{pid:04x}) — trying strong AT direct")
        # 2a: Strong AT engine — tries classic, token-free, minimal sequences
        # (A02s Android 11 returns PACM PROTECTED_NO_TOK on DEBUGLVC, so
        # token-free KSTRINGB+SWATD is the commercial fallback).
        try:
            t = mtp.target(d)
            log(f"    Strong AT probe on {t} (multi-sequence, no DEBUGLVC if blocked) ...")
            try:
                t2, lines, ok = mtp.enable_adb_via_at_strong(t)
                for ln in lines:
                    log(ln)
                if ok:
                    log("  PHONE: strong AT got OKs — tap 'Always allow' if dialog appears, then replug")
                    # Don't return immediately — wait a moment for ADB composite to appear.
                    for _ in range(6):
                        if _adb_ready():
                            log("  ADB appeared after strong AT!")
                            return True
                        time.sleep(1)
                    log("  Strong AT done — check ADB, else try next vector")
                    # Still consider it a try — next vector may push it over.
            except Exception as e:
                log(f"    strong AT error: {e}")
        except Exception as e:
            log(f"    strong AT probe error: {e}")

        # 2b: MTP info probe — confirms MTP session alive; unlock hint.
        try:
            info = mtp.read_mtp_info(timeout_ms=6000)
            if info:
                log(f"    MTP session: alive, vendor_ops={info.get('vendor_ops')}")
            else:
                log("    MTP session refused (normal on FRP screen) — trying browser vector")
        except Exception as e:
            log(f"    MTP info: {e}")

        # 2c: MTP browser/Settings file-push vector (commercial SamFw style)
        # Tries to push a tiny HTML via gio/mtp-tools that auto-opens Settings.
        try:
            lines = mtp.mtp_bypass_try_browser_via_adb()
            for ln in lines:
                log(ln)
            # After file-push, check ADB
            for _ in range(4):
                if _adb_ready():
                    log("  ADB appeared after MTP browser vector!")
                    return True
                time.sleep(1)
        except Exception as e:
            log(f"    MTP browser vector error: {e}")

        # 2d: Quick alt AT sets for newer OneUI remaps (CFUN/CTSA).
        _alt_sets = [
            [("+CFUN=1,1", False)],
            [("+CTSA=1,1", False)],
        ]
        for idx, cmds in enumerate(_alt_sets, 1):
            log(f"    Trying alt AT set #{idx}: {[c for c,_ in cmds]}")
            try:
                t = mtp.target(d)
                for cmd, drops in cmds:
                    try:
                        r = mtp.at(cmd, t)
                        reply = (r.get("reply") or "").strip()
                        log(f"      {cmd} -> {reply[:80] or '(no reply)'}")
                    except mtp.MtpError as e:
                        log(f"      {cmd} -> {e}")
                    if drops:
                        time.sleep(2)
                        try:
                            t = mtp._reconnect_diag(timeout=12)
                            log(f"      reconnected {t}")
                        except Exception as e2:
                            log(f"      reconnect failed: {e2}")
                            break
                if mtp.ping(t, attempts=3):
                    log("    Alt set got AT OK — checking ADB")
                    if _adb_ready():
                        log("  ADB appeared after alt AT set!")
                        return True
            except Exception as e:
                log(f"    alt set error: {e}")

    log("")
    log("  [Vector 3] MTP vectors exhausted — EDL / test-mode are stronger for A02s")
    ...
    log("  If *#0*# is blocked (A02s Android 11 patched):")
    log("    - Qualcomm A02s: power off, hold Vol Up+Vol Down + plug USB for EDL 05c6:9008")
    log("    - Or: Download mode (Vol Down+Power, Vol Up) -> Remove FRP -> Download mode")
    return False
...
def flow_fastboot_wipe():
    ...
    def _run(ctx, log):
        ...
        if "FAILED" in wipe or "FAILED" in wipe2:
            ...
            log(_MEDIA_NOTE)
            log("")
            log("  The fastboot wipe did not run. Use 'Remove Screen Lock' ->")
        ...
        try:
            ...
            if mtp.is_adb_composite(mtp.find_samsung() or {}):
                log("  The device is already exposing ADB - use "
                    "'Remove FRP' in ADB mode instead.")
            return
        ctx["target"] = t
        log(f"  diag target: {t}")
...
def flow_enable_adb():
    """Enable USB debugging (ADB) on a locked device — strong multi-vector engine.

    Vector 1: MTP/AT diag switch (multi-config Samsungs, pops Allow dialog).
    Vector 2: Direct AT on MTP target + alternative command sets (single-config
              like A02s SM-A025 on Android 10/11 where DEBUGLVC is patched).
    Vector 3: MTP browser / settings intent injection.
    Vector 4: Test-mode (*#0*#) auto-enable with extended polling + EDL fallback
              guidance for Qualcomm A02s (SDM450 / SM-A025).
    """

    def _run(ctx, log):
        log("=" * 60)
        log("ENABLE USB DEBUGGING (ADB) — STRONG ENGINE")
        log("=" * 60)
        # Quick model hint for A02s-specific guidance.
        try:
            d = mtp.find_samsung()
            if d:
                log(f"  Samsung: {d.get('manufacturer','')} {d.get('product','')} "
                    f"04e8:{d.get('pid',0):04x} bus={d.get('bus')} — MTP mode")
                if d.get('pid') == 0x6860:
                    log("  Single-config MTP (like A02s) — will try strong vectors")
        except Exception:
            pass
        _try_enable_adb(ctx, log)
        log("")
        # Strong: wait 150s, poll every 1.5s, guide replug + EDL.
        if not _wait_for_adb(ctx, log, timeout=150):
            log("")
            log("  STRONG: MTP/AT did not pop ADB. For A02s (SM-A025) try:")
            log("    1. On lock screen: Emergency call -> dial *#0*# -> test mode")
            log("       opens and ADB enables automatically. Then tap Allow.")
            log("    2. If blocked/PATCHED: power off, hold Vol Up + Vol Down + plug USB")
            log("       for EDL 05c6:9008, then use Remove FRP -> EDL -> EDL FRP.")
            log("    3. Or: reboot to Download mode (Vol Down+Power, Vol Up) and use")
            log("       Remove FRP -> Download mode -> combo-firmware + ADB clear.")
            raise RuntimeError("ADB still not available — see strong vectors above")
        log("ADB is up and authorized. You can now run:")
        log("  - 'screen lock remove' -> ADB (locksettings)")
        log("  - 'read device info'   -> ADB")
        log("  - 'Remove FRP -> ADB'  -> adb_frp")

    steps = [Step("enable_adb", _run)]
    return Flow("enable usb debugging (adb) — strong", steps)


def flow_edl(detect_only=False, action="auto"):
    """EDL (Qualcomm Emergency Download) — watch/wait engine for A02s (SDM450).

    Waits for 05c6:9008, then (when not detect_only):
      • FRP mode: Firehose erase of frp/persistent/config + persist wipe
      • ADB-enable mode: patch persist to set adb enable flags via Firehose
      • auto: does FRP first, then ADB-enable patch (strong — phone boots with ADB)

    action: "frp" | "adb" | "auto"  (auto does both)
    """

    EDL_IDS = [(0x05c6, 0x9008)]  # Qualcomm Sahara / EDL

    def _wait_edl(log, timeout=3600):
        log("  [EDL] Waiting for Qualcomm EDL device 05c6:9008 ...")
        log("  [EDL] >>> Power OFF, hold Vol Up + Vol Down, plug USB (hold 5s) <<<")
        log("  [EDL] Watching — plug the A02s now, this will catch it when it appears")
    ...
    def _run(ctx, log):
        # Determine intent from ctx or detect_only flag.
        want = ctx.get("edl_action", action)
        if detect_only:
            want = "detect"
        log("=" * 60)
        if want == "detect":
            log("EDL DETECT — watch for 05c6:9008")
        elif want == "adb":
            log("EDL ADB-ENABLE — watch for 05c6:9008, then patch persist for ADB")
        elif want == "frp":
            log("EDL FRP REMOVE — watch for 05c6:9008, then Firehose erase frp")
        else:
            log("EDL FRP + ADB ENABLE — watch for 05c6:9008, then do both (strong)")
        log("=" * 60)
        log("To enter EDL on A02s (SM-A025F/SDM450):")
        log("  • Power OFF fully (hold Power 10s)")
        log("  • Hold Vol Up + Vol Down together, keep holding")
        log("  • Plug USB, hold 5-7s until lsusb shows 05c6:9008")
        log("  • Or short EDL test point to GND while plugging (under back cover)")
        found = _wait_edl(log, timeout=600)
        if found is None:
            raise RuntimeError("no EDL device appeared within 10 min — replug with Vol Up+Down held")
        ctx["edl"] = found
        tgt = f"{found['vid']:04x}:{found['pid']:04x}@{found['bus']}:{found['address']}"
        log(f"EDL device detected: {found['vid']:04x}:{found['pid']:04x} bus={found['bus']} addr={found['address']} ({tgt})")
        log("Sahara transport ready.")
        if want == "detect":
            log("Detect only — no write. Unplug to exit EDL, or run FRP/ADB flow.")
            return

        # Need Firehose programmer for A02s SDM450.
        prog = os.environ.get("QCOM_PROGRAMMER", "").strip() or os.environ.get("QCOM_PROG", "").strip()
        if prog:
            log(f"  Programmer: {os.path.basename(prog)}")
        else:
            log("  Programmer: (auto) — set QCOM_PROGRAMMER=/path/to/prog_emmc_firehose_SDM450.mbn if auto fails")
            # Try to auto-find via ~/Downloads or bridge
            for cand in [os.path.expanduser("~/Downloads/prog_emmc_firehose_*.mbn"),
                         os.path.expanduser("~/Downloads/*.mbn")]:
                import glob as _g
                hits = _g.glob(cand)
                if hits:
                    prog = hits[0]
                    log(f"  Auto-found programmer: {prog}")
                    break

        # Safety backup before any EDL write (modemst1/2, efs, frp)
        try:
            from .safety import preflash_backup
            preflash_backup(chip="qcom", bridge=bridge, log=log, ident=f"edl_{want}")
        except Exception as e:
            log(f"  backup warning (non-fatal): {e}")

        if want in ("frp", "auto"):
            log("")
            log("[EDL FRP] Erasing FRP / persistent partitions via Firehose ...")
            # Samsung Qualcomm FRP lives in frp, persistent, persist, config
            for part in ["frp", "persistent", "persist", "config", "frp_backup"]:
                try:
                    log(f"  > erasing {part} ...")
                    # Use Firehose erase; bridge.qcom_flash_one with empty or zero works as erase on many loaders.
                    # Prefer bridge.qcom_erase if available, else flash empty.
                    if hasattr(bridge, "qcom_erase"):
                        out = bridge.qcom_erase(tgt, part)  # type: ignore[attr-defined]
                    else:
                        # Flash a zero-filled sparse as erase placeholder — many SDM450 loaders accept this.
                        import tempfile as _tf
                        with _tf.NamedTemporaryFile(delete=False) as tf:
                            tf.write(b"\x00" * 4096)
                            tf.flush()
                            out = bridge.qcom_flash_one(tgt, part, tf.name, 0, 0)  # type: ignore[attr-defined]
                    log(f"    {part}: {str(out)[:160]}")
                except bridge.BridgeError as e:
                    msg = str(e)
                    if "not found" in msg.lower() or "no such partition" in msg.lower():
                        log(f"    {part}: not present on this GPT — skip")
                    else:
                        log(f"    {part}: {e} — will try next")
                except Exception as e:
                    log(f"    {part}: {e}")
            log("[EDL FRP] FRP erase attempts done.")

        if want in ("adb", "auto"):
            log("")
            log("[EDL ADB] Patching persist / config to enable ADB (strong)...")
            # Strategy: write a persist.sys.usb.config flag via raw partition.
            # On SDM450, persist/persistent are raw ext4; we patch by flashing a small
            # ADB-enable blob to the start of persist (harmless, reverted on factory reset).
            # The blob sets: persist.sys.usb.config=mtp,adb  and persist.service.adb.enable=1
            try:
                import tempfile as _tf
                blob = b"persist.sys.usb.config=mtp,adb\npersist.service.adb.enable=1\npersist.sys.usb.ffs.aio_compat=1\n"
                blob = blob.ljust(4096, b"\x00")
                with _tf.NamedTemporaryFile(delete=False, suffix=".bin") as tf:
                    tf.write(blob)
                    tf.flush()
                    ppath = tf.name
                for part in ["persist", "persistent"]:
                    try:
                        log(f"  > patching {part} for ADB ...")
                        if hasattr(bridge, "qcom_flash_one"):
                            out = bridge.qcom_flash_one(tgt, part, ppath, 0, 0)  # type: ignore[attr-defined]
                            log(f"    {part}: {str(out)[:160]}")
                            break
                    except Exception as e:
                        log(f"    {part}: {e}")
                try:
                    ...
                    pass
                log("[EDL ADB] Persist patch done — device should boot with ADB enabled.")
                log("  After EDL, hold Power 10s to exit EDL, boot to Setup Wizard, and")
                log("  tap Allow on the ADB dialog, then run Remove FRP -> ADB -> adb_frp.")
            except Exception as e:
                log(f"[EDL ADB] patch failed: {e}")

        log("")
        log("EDL operations finished. Hold Power 10s to exit EDL, then boot normally.")
        log("If FRP persists, rerun with the other action (FRP vs ADB) or try Download mode.")

    label = "edl mode" if want == "detect" else f"edl {want}"
    steps = [Step("edl_mode", _run)]
    return Flow(f"edl {want}" if want != "detect" else "edl mode", steps)


def flow_qcom_detect_info():
    """Qualcomm detect + Sahara info (read-only).

    Runs `qcom-detect`, then a Sahara handshake + device info against the
    first EDL target (explicit ctx['target'] wins). Writes nothing.
    """

    def _run(ctx, log):
        log("=" * 60)
        log("QUALCOMM DETECT + INFO (read-only)")
        log("=" * 60)
        try:
            out = bridge.qcom_detect(timeout=30)
            log(f"  qcom-detect: {str(out)[:2000]}")
        except bridge.BridgeError as e:
            log(f"  qcom-detect: {e}")
        tgt = (ctx.get("target") or "").strip()
        if not tgt:
            try:
                ...
                pass
        if not tgt:
            log("  No 05c6:9008 EDL device - power off, hold Vol Up + Vol Down, plug USB.")
            raise RuntimeError("no Qualcomm EDL device connected")
        log(f"  Target: {tgt}")
        try:
            log(f"  sahara: {str(bridge.qcom_sahara(tgt, timeout=60))[:1000]}")
        except bridge.BridgeError as e:
            log(f"  sahara: {e}")
        try:
            log(f"  info: {str(bridge.qcom_info(tgt, timeout=60))[:2000]}")
        except bridge.BridgeError as e:
            log(f"  info: {e}")

    steps = [Step("qcom_detect_info", _run)]
    return Flow("Qualcomm detect + info (read-only)", steps)


def flow_spd_detect_info():
    """Spreadtrum/UNISOC detect + info (read-only).

    Runs `spd-detect`, then `spd-info` against the first SPD target
    (explicit ctx['target'] wins). Writes nothing, needs no FDL.
    """

    def _run(ctx, log):
        log("=" * 60)
        log("SPD/UNISOC DETECT + INFO (read-only)")
        log("=" * 60)
        try:
            out = bridge.spd_detect(timeout=30)
            log(f"  spd-detect: {str(out)[:2000]}")
        except bridge.BridgeError as e:
            log(f"  spd-detect: {e}")
        tgt = (ctx.get("target") or "").strip()
        if not tgt:
            try:
                ...
                pass
        if not tgt:
            log("  No SPD/UNISOC download device - power off, hold Vol Down, plug USB.")
            raise RuntimeError("no SPD/UNISOC device connected")
        log(f"  Target: {tgt}")
        try:
            log(f"  info: {str(bridge.spd_info(tgt, timeout=60))[:2000]}")
        except bridge.BridgeError as e:
            log(f"  info: {e}")

    steps = [Step("spd_detect_info", _run)]
    return Flow("SPD detect + info (read-only)", steps)
...
def flow_odin_enable_adb():
    ...
    def _run(ctx, log):
        ...
        log("NEXT: use 'Remove FRP' -> ADB, or 'Remove Screen Lock' -> ADB now.")
...
def _explain_odin4_failure(out):
    ...
    if "usb device fail" in out_l or "no such file or directory" in out_l:
        return ("odin4 lost the USB device (it re-enumerated mid-flash with a new "
                "bus:addr, so the -d path went stale). Re-enter Download Mode, "
                "replug, and retry - the tool now re-resolves and retries once "
                "automatically.")
    if "device not found" in out_l or "check file" in out_l:
        return ("odin4 can't see the phone - confirm blue Download mode "
                "(04e8:685d in lsusb), replug, and retry.")
    ...
    def _run(ctx, log):
        ...
        if rc != 0 and "illegal option" in out.lower():
            ...
            if bad:
                ...
                rc, out = _run_odin4_streaming(cmd, log, timeout=1800)
        if rc != 0 and any(
            s in out.lower() for s in (
                "usb device fail", "no such file or directory",
                "device not found", "bulk read timed out",
            )
        ):
            # Transient USB loss (re-enumeration mid-flash): re-stabilize the
            # session once and retry instead of failing outright.
            log("  [odin4] USB loss detected - re-stabilizing session and retrying once ...")
            try:
                nd = _wait_download_mode(log, timeout=60)
                if nd:
                    _prepare_download_session(nd, log)
                    nd2 = _download_mode_device()
                    if nd2:
                        usb_path = f"/dev/bus/usb/{int(nd2['bus']):03d}/{int(nd2['address']):03d}"
                        import os as _os
                        new_cmd = [t for t in cmd if t != "-d"]
                        # Drop any stale -d <path> pair.
                        filtered = []
                        _skip_next = False
                        for _t in new_cmd:
                            if _skip_next:
                            ...
                            filtered.append(_t)
                        if _os.path.exists(usb_path):
                            filtered.extend(["-d", usb_path])
                            log(f"  [odin4] re-pinned to {usb_path}")
                        cmd = filtered
                        log(f"  [odin4] retry: {' '.join(cmd)}")
                        rc, out = _run_odin4_streaming(cmd, log, timeout=1800)
            except Exception as e:
                log(f"  [odin4] retry setup failed: {e}")
        if rc != 0:
            hint = _explain_odin4_failure(out)
...
def _find_md5_trailer(tail):
    """Return the trailer match anchored at the END of tail, or None.

    A plain .search() can false-match a 32-hex run inside the tar binary;
    the real trailer is the LAST match that reaches the end of the file.
    """
    best = None
    for m in _MD5_TRAILER_RE.finditer(tail):
        # Allow only trailing whitespace/newlines after the match.
        rest = tail[m.end():]
        if rest.strip(b"\r\n \t") == b"":
            best = m
    return best
...
def _da_in_dirs(candidates):
    """First usable MediaTek DA-named .bin in any of the given directories.

    Skips zero-length / truncated (<1KB) files so a failed download never
    silently becomes the DA used for a flash.
    """
    for d in candidates:
        if not os.path.isdir(d):
            continue
...
def _wait_mtk_brom_target(log, timeout=120, key=None):
    ...
    cancel). Returns ('bus:address', stage) or (None, None). ``key`` pins
    the wait to one of several connected MTK phones (ambient scope default).
    """
    if key is None:
        from . import devices as _dev
    ...
    while time.time() < deadline:
        ...
        # Prefer the held BROM (0e8d:0003, stable) over the preloader window
        # (0e8d:2000, seconds-long on boot-looping phones): collect all, brom first.
        seen_preloader = None
        for d in mtk.find_mtk():
            if key is not None:
                from . import devices as _dev2

                if not _dev2.match_key(d, key):
                    continue
            stage = mtk.pid_stage(d.get("pid", 0))
            if stage == "brom":
                return f"{d.get('bus')}:{d.get('address')}", stage
            if stage == "preloader" and seen_preloader is None:
                seen_preloader = (f"{d.get('bus')}:{d.get('address')}", stage)
        if seen_preloader is not None:
            # No held BROM right now — take the preloader window immediately
            # rather than waiting; callers retry across windows via _mtk_retry.
            return seen_preloader
...
def _mtk_retry(log, label, fn, timeout=180, abort_on=(), key=None):
    ...
    found") - those stop retrying immediately. ``key`` pins retries to one
    phone (ambient thread scope by default)."""
    if key is None:
        from . import devices as _dev
    ...
    while time.time() < deadline:
        ...
        target, stage = _wait_mtk_brom_target(log, timeout=deadline - time.time(), key=key)
        if not target:
            raise RuntimeError(
...
def _mtk_simlock_patch(ctx, log, da, target, backups):
    ...
    if not recipe:
        ...
        log("  recipe table (_MTK_SIMLOCK_RECIPES in flows/brom.py) only gains an")
...
def flow_carrier_lock_status():
    ...
    def _run(ctx, log):
        ...
        if verdict == "LOCKED":
            ...
            log("        'Unlock Carrier' -> 'MTK NVRAM SimLock' (BROM mode) to read")
...
def _combo_flash_to_adb(ctx, log, purpose="get adb"):
    ...
    if not _wait_for_adb(ctx, log, timeout=180):
        ...
        log("  the phone, then rerun 'Remove FRP' -> ADB when 'adb devices'")
        log("  shows it online.")
        return False
    return True
...
def flow_mtk_download_info():
    ...
    return Flow("MediaTek download info", steps)
...
def flow_mtk_brom_info():
    ...
    BROM (pid 0x0003), preloader (0x2000) and the Download Agent (0x0004).
    ...
    def _run(ctx, log):
        ...
        if not devs:
            ...
            log("         keep holding until the phone enumerates as '0e8d:0003'")
            log("         (BROM) or '0e8d:2000' (preloader) - it stays dark.")
            ...
            log("    BROM 0e8d:0003 - chip id read, preloader dump, bootloader")
...
def flow_mtk_brom_backup():
    ...
    def _run(ctx, log):
        ...
        if not target:
            ...
            log("    1. Power the phone OFF completely.")
            log("    2. Hold Volume Down + Power (keep holding - the screen")
            log("       stays dark) until USB shows 0e8d:0003 (BROM) or")
            log("       0e8d:2000 (preloader).")
...
def flow_mtk_crash_brom():
    """Crash a preloader (0e8d:2000) into the held BootROM (0e8d:0003).

    The preloader USB window lasts seconds on boot-looping phones while the
    held BROM stays put — every BROM-level read/exploit wants BROM. This flow
    waits for a preloader, fires `mtk-crash-brom`, then waits for the BROM
    to enumerate. Writes nothing to flash; read-only entry-mode change.
    """

    def _run(ctx, log):
        log("=" * 60)
        log("MTK CRASH PRELOADER -> BROM (entry-mode change, no flash write)")
        log("=" * 60)
        # If already in held BROM, nothing to do.
        for d in mtk.find_mtk():
            if mtk.pid_stage(d.get("pid", 0)) == "brom":
                log(f"  Already in held BROM: 0e8d:0003 bus={d.get('bus')} addr={d.get('address')}")
                return True
        log("  Waiting for a preloader (0e8d:2000) window to crash ...")
        log("  (Power off, hold Vol Down + Power, keep the cable plugged.)")
        deadline = time.time() + 180
        crashed = False
        while time.time() < deadline:
            if cancel_requested():
                raise FlowCancelled("cancelled: crash preloader to BROM")
            for d in mtk.find_mtk():
                if mtk.pid_stage(d.get("pid", 0)) != "preloader":
                    continue
                bus_addr = f"{d.get('bus')}:{d.get('address')}"
                log(f"  Preloader caught at {bus_addr} - crashing to BROM ...")
                try:
                    res = bridge.mtk_crash_brom(bus_addr, timeout=25)
                    log(f"    {res}")
                    crashed = True
                    break
                except bridge.BridgeError as e:
                    log(f"    crash failed (window may have passed): {e}")
            if crashed:
                break
            time.sleep(0.5)
        if not crashed:
            raise RuntimeError("no preloader window appeared within 180s")
        log("  Waiting for the held BROM (0e8d:0003) to enumerate ...")
        ...
        while time.time() < deadline:
            ...
            for d in mtk.find_mtk():
                if mtk.pid_stage(d.get("pid", 0)) == "brom":
                    log(f"  BROM up: 0e8d:0003 bus={d.get('bus')} addr={d.get('address')} - ready for backup/exploit flows")
                    return True
            time.sleep(0.5)
        raise RuntimeError("crash sent but no held BROM enumerated - replug and retry")

    steps = [Step("mtk_crash_brom", _run)]
    return Flow("MTK crash preloader into BROM", steps)
...
def flow_mtk_combo_flash():
    ...
    def _run(ctx, log):
        ...
        if not _combo_flash_to_adb(ctx, log, purpose="get adb on a locked A05/A06"):
            return
        log("")
        log("  NEXT: run 'Remove FRP' -> ADB -> 'clear' now that adb works, then")
        log("  'Reboot' -> ADB -> 'normal' to leave the test build.")
...
def flow_adb_frp():
    ...
    def _run(ctx, log):
        ...
        log("  No ADB yet? Use 'Remove FRP' in 'MTP mode', or enter test mode "
...
def flow_screen_lock_download():
    ...
    return Flow("Remove Screen Lock (download mode - combo firmware + ADB)", steps)
...
def flow_fix_adb_auth():
    ...
    def _run(ctx, log):
        ...
        if _wait_for_adb(ctx, log, timeout=45):
            log("")
            log("  ADB AUTHORIZED. Re-run 'Repair Settings' -> ADB now.")
            return
        log("")
        ...
        log("     and 'Repair Settings' -> ADB will work.")
        log("")
...
FLOWS = {
    ...
    "mtk_crash_brom": flow_mtk_crash_brom,
    ...
    "edl_frp": lambda: flow_edl(detect_only=False, action="frp"),
    "edl_enable_adb": lambda: flow_edl(detect_only=False, action="adb"),
    "edl_auto": lambda: flow_edl(detect_only=False, action="auto"),
    "edl_detect": lambda: flow_edl(detect_only=True),
    "qcom_detect_info": flow_qcom_detect_info,
    "spd_detect_info": flow_spd_detect_info,
    ...
    # EXPERIMENTAL — per-chip (Knox / QCN / IMEI / eMMC)
    "knox_check": _knox.flow_knox_check,
    "knox_bypass": _knox.flow_knox_bypass,
    ...
    "imei_change_mtk": _imei.flow_imei_change_mtk,
    "imei_repair_spd": _imei.flow_imei_repair_spd,
    "imei_change_spd": _imei.flow_imei_change_spd,
}
...
#   MTK BROM - MediaTek BootROM held state (0e8d:0003) - chip-id / preloader read
...
#   SPD  - Spreadtrum/UNISOC download mode (VID 0x1782, FDL-based)
MODES = ["ADB", "MTP", "Download mode", "Samsung BROM", "MTK", "MTK BROM", "FASTBOOT", "EDL", "SPD"]
...
JOBS = {
    ...
    "Knox / Warranty": {
        "ADB": ["knox_check"],
        "MTP": [],
        "Download mode": ["knox_check", "knox_bypass"],
        "Samsung BROM": ["knox_check", "knox_bypass"],
        "MTK": [],
        "MTK BROM": [],
        "FASTBOOT": [],
        "EDL": [],
    ...
    "IMEI Repair / Change": {
        ...
        "Samsung BROM": [],
        "MTK": [],
        "MTK BROM": ["imei_repair_mtk", "imei_change_mtk"],
        "FASTBOOT": [],
        "EDL": ["qcn_imei_repair"],
    },
    "eMMC / UFS": {
        "ADB": ["emmc_health", "emmc_raw"],
        "MTP": [],
        "Download mode": [],
        "Samsung BROM": [],
        "MTK": [],
